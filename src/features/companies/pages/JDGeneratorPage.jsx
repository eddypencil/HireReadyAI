    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-6 font-sans">
        <div className="text-center max-w-sm">
          ...
          <h2 className="text-sidebar text-xl font-bold mb-1.5">Job Published!</h2>
          <p className="text-muted-foreground/80 text-xs font-medium leading-relaxed mb-6">
            <span className="font-bold text-sidebar">{title}</span> has been published and is now visible to applicants.
          </p>
          <button
            onClick={() => {
              setPublished(false);
  ...
  return (
    <div className="min-h-screen bg-background p-4 sm:p-6 font-sans">
      ...
      <div className="max-w-6xl mx-auto mb-4 flex items-center justify-between gap-4">
        <div>
          <h1 className="text-base font-bold text-sidebar">Job Description Generator</h1>
        </div>
        ...
        <div className="bg-background rounded-xl border border-border/60 p-5 shadow-xs">
          ...
          <form onSubmit={handleGenerate} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              />
              {errors.title && <p className="text-xs font-medium text-destructive mt-0.5">{errors.title}</p>}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-1.5">
                ...
                {errors.seniority && <p className="text-xs font-medium text-destructive mt-0.5">{errors.seniority}</p>}
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                {errors.jobType && <p className="text-xs font-medium text-destructive mt-0.5">{errors.jobType}</p>}
              </div>
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {errors.workLocation && <p className="text-xs font-medium text-destructive mt-0.5">{errors.workLocation}</p>}
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              />
              {errors.experienceYears && <p className="text-xs font-medium text-destructive mt-0.5">{errors.experienceYears}</p>}
            </div>

            <div className="flex flex-col gap-1.5">
              <label className={labelClass}>
                Required skills <span className="text-muted-foreground/50 normal-case font-normal">(optional)</span>
              </label>
              <input
                type="text"
            ...
            <div className="flex flex-col gap-1.5">
              <label className={labelClass}>
                Salary range (EGP) <span className="text-muted-foreground/50 normal-case font-normal">(optional)</span>
              </label>
              <div className="flex items-center gap-2">
                <input
                  type="number"
                  ...
                  className={inputClass}
                />
                <span className="text-muted-foreground/60 text-xs shrink-0 font-medium">to</span>
                <input
                  type="number"
                  value={salaryMax}
            ...
            <div className="flex flex-col gap-1.5">
              <label className={labelClass}>
                Key notes <span className="text-muted-foreground/50 normal-case font-normal">(optional)</span>
              </label>
              <textarea
                rows={4}
                value={keyNotes}
        ...
        <div className="bg-background rounded-xl border border-border/60 p-5 shadow-xs min-h-[450px] lg:h-full">
          {!generated ? (
            ...
            <div className="space-y-5">
              ...
              <div>
                <h2 className="text-base font-bold text-sidebar">{previewMeta.title}</h2>
              ...
              {aiResult?.responsibilities?.length > 0 && (
                <div className="border-t border-border/40 pt-4">
                  <h3 className="text-xs font-bold text-sidebar mb-2">What you'll do</h3>
                  <ul className="space-y-1.5">
                    {aiResult.responsibilities.map((item, i) => (
                      <li key={i} className="flex items-start gap-2 text-muted-foreground/90 text-xs font-medium leading-relaxed">
              ...
              {aiResult?.requirements?.length > 0 && (
                <div className="border-t border-border/40 pt-4">
                  <h3 className="text-xs font-bold text-sidebar mb-2">What we're looking for</h3>
                  <ul className="space-y-1.5">
              ...
              {aiResult?.skills?.length > 0 && (
                <div className="border-t border-border/40 pt-4">
                  <h3 className="text-xs font-bold text-sidebar mb-2.5">Skills & Tools</h3>
                  <div className="grid grid-cols-2 gap-x-2 gap-y-1.5">
                    {aiResult.skills.map((skill, i) => (
                      <div key={i} className="flex items-center gap-1.5 text-xs font-medium text-muted-foreground/90">

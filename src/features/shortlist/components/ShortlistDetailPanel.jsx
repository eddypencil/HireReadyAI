...
import {
  ThumbsUp, ThumbsDown, Minus, Sparkles, X,
  ChevronUp, Send, Clock, UserCheck,
} from "lucide-react";
...
function getAvatarColor(name = "") {
  const colors = ["bg-violet-500","bg-sky-500","bg-emerald-500","bg-amber-500","bg-rose-500","bg-indigo-500","bg-teal-500","bg-fuchsia-500"];
  let hash = 0;
...
const VOTE_CONFIG = {
  up:      { label: "Up",      Icon: ThumbsUp,   active: "bg-emerald-500 text-white",  inactive: "bg-gray-100 text-gray-500 hover:bg-emerald-50 hover:text-emerald-600" },
  neutral: { label: "Neutral", Icon: Minus,       active: "bg-gray-500 text-white",     inactive: "bg-gray-100 text-gray-500 hover:bg-gray-200" },
  down:    { label: "Down",    Icon: ThumbsDown,  active: "bg-red-500 text-white",      inactive: "bg-gray-100 text-gray-500 hover:bg-red-50 hover:text-red-500" },
};
      ...
      >
        ...
        <div className="px-5 pt-5 pb-4 border-b border-gray-100 bg-white shrink-0">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className={`w-11 h-11 rounded-full flex items-center justify-center text-white text-sm font-bold shrink-0 ${getAvatarColor(candidate?.full_name)}`}>
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {composite_score != null && (
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full border ${
                      composite_score >= 80 ? "bg-emerald-50 text-emerald-700 border-emerald-200" :
                      composite_score >= 65 ? "bg-amber-50 text-amber-700 border-amber-200" :
                ...
                <p className="text-xs text-gray-500">{candidate?.headline || candidate?.role}</p>
                <div className="flex flex-wrap gap-1 mt-1">
                  {tags.map((tag) => (
                    <span key={tag} className={`text-[10px] px-2 py-0.5 rounded-full border font-medium ${TAG_COLORS[tag] || "bg-gray-100 text-gray-600 border-gray-200"}`}>
                      {tag}
                    </span>
                  ))}
                </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="px-5 py-4 border-b border-gray-100">
            <p className="text-[10px] font-bold tracking-widest uppercase text-gray-400 mb-3">Your Vote</p>
            {myVote ? (
              <div className={`rounded-lg px-4 py-3 text-sm font-medium mb-3 ${
                myVote === "up" ? "bg-emerald-50 text-emerald-700 border border-emerald-200" :
                myVote === "down" ? "bg-red-50 text-red-600 border border-red-200" :
                "bg-gray-50 text-gray-600 border border-gray-200"
              }`}>
                You voted <strong className="capitalize">{myVote}</strong> — tap again to undo
              </div>
            ) : (
              <p className="text-xs text-gray-400 mb-3">You haven't voted yet.</p>
            )}
            <div className="flex gap-2">
              {Object.entries(VOTE_CONFIG).map(([value, { label, Icon, active, inactive }]) => (
                <button
                  key={value}
                  onClick={() => onCastVote(app.id, myVote === value ? null : value)}
          ...
          <div className="px-5 py-4 border-b border-gray-100">
            <div className="flex items-center justify-between mb-3">
              <p className="text-[10px] font-bold tracking-widest uppercase text-gray-400">Team Votes</p>
              <span className="text-xs text-gray-500">{totalVoters} / ? cast</span>
            </div>
            {totalVoters > 0 && (
              <div className="w-full bg-gray-100 rounded-full h-1.5 mb-3 overflow-hidden">
                <div
                  className="h-1.5 rounded-full bg-emerald-400 transition-all"
                  style={{ width: `${(upVotes / Math.max(totalVoters, 1)) * 100}%` }}
                />
              </div>
            )}
            <div className="space-y-2">
              {votes.map((v, i) => (
                <div key={v.id || i} className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className={`w-7 h-7 rounded-full text-[10px] font-bold flex items-center justify-center text-white ${getAvatarColor(v.profiles?.full_name)}`}>
                      {getInitials(v.profiles?.full_name)}
                    </div>
                    <div>
                      <p className="text-xs font-medium text-gray-800">{v.profiles?.full_name}</p>
                      <p className="text-[10px] text-gray-400">{v.profiles?.headline || v.profiles?.role}</p>
                    </div>
                  </div>
                  <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${
                    v.vote === "up" ? "bg-emerald-50 text-emerald-600 border border-emerald-200" :
                    v.vote === "down" ? "bg-red-50 text-red-500 border border-red-200" :
            ...
            <div className="bg-mauve-magic-50 border border-mauve-magic-200 rounded-xl p-4">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-1.5">
                  ...
                  <p className="text-[10px] font-bold tracking-widest uppercase text-mauve-magic-600">AI Rationale</p>
                </div>
                {ai_confidence != null && (
          ...
          {/* TEAM NOTES */}
          <div className="px-5 py-4 border-b border-gray-100">
            <p className="text-[10px] font-bold tracking-widest uppercase text-gray-400 mb-3">Team Notes</p>
            {notesLoading ? (
              <div className="text-xs text-gray-400">Loading notes...</div>
            ) : (
              <div className="space-y-3 mb-3">
                {notes.length === 0 && (
                  <p className="text-xs text-gray-400">No notes yet. Be the first to leave one.</p>
                )}
                {notes.map((note) => (
        ...
        {!is_rejected ? (
          <div className="px-5 py-4 border-t border-gray-100 bg-white shrink-0 space-y-2">
            {showRejectInput ? (
              <div className="space-y-2">
                <p className="text-xs text-gray-500 font-medium">Rejection reason (pre-filled from AI):</p>
                <textarea
                  rows={3}
                  value={rejectReason}
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-gradient-to-r from-dark-amethyst-600 to-mauve-magic-600 hover:from-dark-amethyst-700 hover:to-mauve-magic-700 text-white rounded-xl text-sm font-semibold transition-all shadow-sm"
                >
          ...
          <div className="px-5 py-4 border-t border-gray-100 bg-red-50 shrink-0">
            <p className="text-xs text-red-500 font-semibold mb-1">This candidate was rejected</p>
            {rejection_reason && <p className="text-xs text-gray-500 leading-relaxed">{rejection_reason}</p>}
          </div>
        )}
      </div>
    </>

...
import {
  ChevronRight, ChevronDown, FileText, Video, Code, ListChecks,
  Clock, MessageSquare, Check, X, Sparkles
} from "lucide-react";

const TYPE_ICONS = { video: Video, text: FileText, code: Code, multiple_choice: ListChecks };
const TYPE_LABELS = { video: "Video Response", text: "Written Answer", code: "Code Challenge", multiple_choice: "Multiple Choice" };
const TYPE_ICON_BG = {
...
export default function ExpandableQuestion({ question, index }) {
  ...
  const iconBg = TYPE_ICON_BG[question.question_type] || "bg-surface-muted text-accent";
                ...
                <div className="flex items-center gap-2">
                  ...
                  {context.code_type && (
                    <span className="px-2 py-0.5 rounded text-xs font-bold bg-surface-muted text-accent">
                      {context.code_type === "visuals" ? "UI / Visual" : "Problem Solving"}
                    </span>
                  )}
                </div>
                    ...
                    {options.map((opt, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span>{opt}</span>
                          {isSelected && <Check className="w-3.5 h-3.5 ml-auto text-accent" />}
                        </div>
                      );
                    })}
                  </div>
                ...
                <div className="mb-2">
                  ...
                  <ul className="space-y-1">
                    {answerData.strengths.map((s, i) => (
                      <li key={i} className="flex items-start gap-1.5 text-xs text-success">

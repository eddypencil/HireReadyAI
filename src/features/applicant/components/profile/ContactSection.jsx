import { Mail, Phone, MapPin, Globe, ExternalLink, Pencil, Check } from "lucide-react";
import SectionCard from "./SectionCard";
import { InputField } from "./FormFields";

export default function ContactSection({
  profile, user, isOwn,
  editBasic, setEditBasic, savingBasic,
  onFieldChange, onSaveBasic, onCancelBasic,
}) {
  if (editBasic) {
    return (
      <SectionCard icon={null} title="Contact Information">
        <div className="space-y-3">
          <InputField label="Full Name" value={profile.full_name || ""} onChange={(v) => onFieldChange("full_name", v)} placeholder="Your name" />
          <InputField label="Headline" value={profile.headline || ""} onChange={(v) => onFieldChange("headline", v)} placeholder="e.g. Frontend Developer" />
          <InputField label="Phone" value={profile.phone || ""} onChange={(v) => onFieldChange("phone", v)} placeholder="+20 10 0000 0000" />
          <InputField label="Location" value={profile.location || ""} onChange={(v) => onFieldChange("location", v)} placeholder="City, Country" />

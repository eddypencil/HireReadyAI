  ...
  return (
    <div className="flex items-start gap-3 flex-1">
      ...
      <div className="flex-1">
        ...
        {item.image && <img src={item.image} alt={item.name} className="mt-2 max-w-[160px] rounded-lg border border-border/60 cursor-pointer hover:opacity-80 transition-opacity" onClick={() => onImageClick?.(item.image)} />}
      </div>
    </div>
  );
}
    ...
    <div className="flex-1">
      ...
      {item.images?.length > 0 && (
        <div className="flex flex-wrap gap-2 mt-2">
          {item.images.map((img, ii) => <img key={ii} src={img} alt="" className="w-20 h-16 rounded-lg object-cover border border-border/60 cursor-pointer hover:opacity-80 transition-opacity" onClick={() => onImageClick?.(img)} />)}
        </div>
      )}
    </div>
  );
...
export default function ArraySection({
  icon, title, section,
  items = [], isOwn,
  onEdit, onDelete, onAdd, onImageClick,
}) {
  const ItemComponent = ITEM_RENDERERS[section];

  return (
    <SectionCard icon={icon} title={title}>
      {items.length === 0 && <EmptyState message={`No ${title.toLowerCase()} added yet`} />}
      <div className="space-y-3">
        {items.map((item, i) => (
          <div key={i} className="group flex items-start justify-between gap-2">
            {ItemComponent && <ItemComponent item={item} onImageClick={onImageClick} />}
            {isOwn && (
              <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => onEdit(section, i)} className="p-1 text-muted-foreground hover:text-primary transition-colors"><Pencil className="w-3.5 h-3.5" /></button>
                <button onClick={() => onDelete(section, i)} className="p-1 text-muted-foreground hover:text-destructive transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
              </div>
            )}
          </div>
        ))}
      </div>
      {isOwn && <button onClick={() => onAdd(section)} className="mt-3 flex items-center gap-1.5 text-xs font-medium text-primary hover:text-primary/80 transition-colors"><Plus className="w-3.5 h-3.5" />Add {title.toLowerCase()}</button>}
    </SectionCard>
  );
}

...
export default function DialogForms({ dialog, handleDialogChange, errors = {} }) {
    ...
    return (
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <InputField label="Start Date" type="month" value={data.from || ""} onChange={(v) => handleDialogChange("from", v)} error={errors.from} />
          {data.to !== "present" && (
            <InputField label="End Date" type="month" value={data.to || ""} onChange={(v) => handleDialogChange("to", v)} error={errors.to} />
          )}
        </div>
  ...
  if (section === "education") {
    return (
      <div className="space-y-3">
        <div>
          <label className={`block text-xs font-medium mb-1 ${errors.level ? "text-destructive" : "text-muted-foreground"}`}>Degree Level</label>
          <select
            value={data.level || ""}
        ...
        <div className="grid grid-cols-2 gap-3">
          <InputField label="Start Year" type="number" value={data.start_year || ""} onChange={(v) => handleDialogChange("start_year", v)} placeholder="Start year" error={errors.start_year} />
          {data.end_year !== "present" && (
            <InputField label="End Year" type="number" value={data.end_year || ""} onChange={(v) => handleDialogChange("end_year", v)} placeholder="End year" error={errors.end_year} />
          )}
        </div>
  ...
  if (section === "certificates") {
    return (
      <div className="space-y-3">
        ...
        />
        <div className="grid grid-cols-2 gap-3">
          <InputField label="Date" type="month" value={data.date || ""} onChange={(v) => handleDialogChange("date", v)} error={errors.date} />
  ...
  if (section === "volunteering") {
    return (
      <div className="space-y-3">
        ...
        />
        <div className="grid grid-cols-2 gap-3">
          <InputField label="Start Date" type="month" value={data.start || ""} onChange={(v) => handleDialogChange("start", v)} error={errors.start} />
          <InputField label="End Date" type="month" value={data.end || ""} onChange={(v) => handleDialogChange("end", v)} error={errors.end} />
        </div>
        <InputField label="Description" value={data.description || ""} onChange={(v) => handleDialogChange("description", v)} placeholder="Description..." multiline />
      </div>
    );

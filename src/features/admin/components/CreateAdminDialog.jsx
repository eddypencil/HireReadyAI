...
export default function CreateAdminDialog({ onClose }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setError(err.message || t("admin.create_admin.failed"));
    } finally {
      setSubmitting(false);
    }
  }
      ...
      <div className="bg-card rounded-2xl border border-border shadow-2xl w-full max-w-md mx-4">
        <div className="flex items-center justify-between p-4 border-b border-border">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-foreground">{t("admin.create_admin.title")}</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-foreground">{t("admin.full_name")}</label>
            <input
              type="text"
              value={fullName}
              onChange={(e) => setFullName(e.target.value)}
              placeholder={t("admin.create_admin.full_name_placeholder")}
              required
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-foreground">{t("admin.email")}</label>
            <input
              type="email"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              placeholder={t("admin.create_admin.email_placeholder")}
              required
              className="w-full bg-background border border-border rounded-xl px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/40"
            />
          </div>
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-foreground">{t("admin.password")}</label>
            <input
              type="password"
              value={password}
              onChange={(e) => setPassword(e.target.value)}
              placeholder={t("admin.create_admin.password_placeholder")}
              required
              minLength={8}
              className="w-full bg-background border border-border rounded-xl px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/40"
            />
          ...
          <div className="flex gap-2 pt-2">
            ...
            >
              ...
              {submitting ? t("admin.creating") : success ? t("admin.create_admin.created") : t("admin.create_admin.title")}
            </button>
          </div>
        </form>
      </div>
